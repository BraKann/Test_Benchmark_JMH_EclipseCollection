...
Description :
-------------
Ce script implémente une campagne expérimentale pour mesurer
la consommation énergétique de différentes versions d’Eclipse Collections
à l’aide de benchmarks JMH et des métriques Kwollect sur Grid5000.

Objectif :
----------
Fournir un protocole rigoureux et reproductible permettant :
  - la comparaison fiable de versions logicielles,
  - l’analyse statistique de la consommation énergétique,
  - la visualisation des résultats (graphes, histogrammes).

Pipeline expérimental :
----------------------
1. Warmup machine :
   - Exécution préalable d’un benchmark pour stabiliser l’état matériel
     (température CPU, fréquence, JIT).

2. Campagne principale :
   - Build de chaque version.
   - Exécution de plusieurs itérations JMH.
   - Mesure d’une phase idle avant chaque run.

3. Collecte des métriques :
   - Wattmètre (consommation globale).
   - BMC (estimation interne).
   - RAPL (CPU, DRAM, uncore) si disponible.

4. Post-traitement :
   - Calcul de statistiques (moyenne, écart-type, min, max).
   - Calcul de l’énergie nette (soustraction du bruit idle).
   - Agrégation des résultats par version.

5. Visualisation :
   - Histogrammes (puissance, énergie).
   - Barres d’erreur (variance).
   - Décomposition énergétique (CPU / DRAM / cache).
   - Courbes d’évolution.

Caractéristiques :
-----------------
- Support multi-métriques (wattmètre + RAPL).
- Génération automatique de fichiers JSON et CSV.
- Analyse et graphiques intégrés (matplotlib).
