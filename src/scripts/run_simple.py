...
Description :
-------------
Ce script exécute une campagne simplifiée de benchmarks JMH sur différentes versions
d’Eclipse Collections, avec une collecte simplifiée de la consommation
énergétique via Kwollect (Grid5000).

Objectif :
----------
Fournir un outil léger et rapide pour :
  - tester plusieurs versions d’une librairie,
  - mesurer la consommation énergétique globale (wattmètre),
  - produire des résultats exploitables (JSON).

Fonctionnement :
---------------
Pour chaque version :
  1. Build du projet avec Gradle.
  2. Exécution des benchmarks JMH.
  3. Mesure d’une phase idle (consommation de base).
  4. Collecte des données Kwollect (puissance, énergie).
  5. Calcul d’une énergie nette (énergie totale - bruit de fond).

Caractéristiques :
-----------------
- Une seule métrique principale : wattmetre_power_watt.
- Pas de phase de warmup machine globale.
- Structure de sortie simple (campaign-summary.json).
- Pas d’analyse statistique ni de visualisation intégrée.

Limitations :
-------------
- Sensible aux variations matérielles (température, CPU scaling).
- Peu de métriques (pas de détail CPU / DRAM / cache).
- Analyse externe nécessaire pour exploiter les résultats.
